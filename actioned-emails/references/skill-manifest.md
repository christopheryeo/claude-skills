# Skill Creation Manifest: actioned-emails

## Context and Design Process
This manifest was prepared by following the workflow in `references/instructions.md` alongside patterns established in:
- `recent-emails/skill.md` for timeline-driven Gmail recaps.
- `starred-email/skill.md` for priority filtering and follow-up surfacing.

## Overview
Deliver a combined Gmail briefing that highlights what the user **sent** recently and what they still have **starred**. The skill merges both perspectives into one executive digest so the user can validate recent outbound activity and stay on top of outstanding follow-ups without running two separate skills.

## Success Criteria and Guard Rails
- **Success looks like:**
  - Uses Gmail integrations (`search_gmail_messages`, `read_gmail_thread`) to pull sent and starred messages within the requested windows.
  - Defaults to 24 hours for sent mail and 7 days for starred mail when the user provides no timeframe, while supporting explicit overrides.
  - Deduplicates overlapping messages (e.g., sent mail that is also starred) and tags them clearly.
  - Produces an executive summary, merged table, and follow-up list with ≤30-word summaries and actionable notes.
  - Provides Gmail deep links and timezone-aware timestamps for every entry.
- **Guard rails:**
  - Never fabricate email content or metadata; rely solely on Gmail tools.
  - Keep the output read-only—do not alter star status or labels.
  - Maintain discretion by avoiding unnecessary sensitive details in summaries.
  - Explain integration failures transparently and request user assistance to retry.

## Step 1: Create Directory Structure
Run the following commands to scaffold the skill:

```bash
mkdir -p actioned-emails/references
```

## Step 2: Create SKILL.md
**File path:** `actioned-emails/skill.md`

Copy the content below verbatim into `skill.md`.
...
## Step 3: References
No additional reference documents are required beyond this manifest. Leave the `references/` directory as-is when packaging.

## Step 4: Assets
This skill does not require assets. Do not add files under `assets/`.

## Validation & Next Steps
- Confirm that `actioned-emails/skill.md` matches the content above exactly.
- Test the prompt with variations like "What did I send today and what have I starred?" to ensure the instructions trigger correctly.
- Compress the `actioned-emails` folder (or integrate into your skills workspace) once validated.
- Revisit the skill after user feedback to refine default windows or add additional metrics if needed.
